...
export const users = sqliteTable("users", {
  ...
  // Set by better-auth's anonymous plugin for guest sessions.
...
export const notifications = sqliteTable("notifications", {
  id: text("id").primaryKey(),
  userId: text("user_id").notNull().references(() => users.id, { onDelete: "cascade" }),
  type: text("type").notNull(), // invite, instance, billing, system
...
// Cloud credentials - the AWS (or other provider) keys CCCP uses to drive a
// user's infrastructure. The secret is never stored in plaintext: it is sealed
// with AES-256-GCM by lib/crypto.ts before it reaches this table, and it is
// only ever unsealed server-side, inside an API route that has already
// established a session for the owning user.
export const cloudCredentials = sqliteTable(
  "cloud_credentials",
  {
    id: text("id").primaryKey(),
    userId: text("user_id")
    ...
    provider: text("provider").notNull().default("aws"), // aws, (future: gcp, azure, hetzner)
    label: text("label").notNull().default("Default"),
    region: text("region").notNull().default("us-east-1"),

    // Public half of the key pair - safe to show in the UI.
    accessKeyId: text("access_key_id").notNull(),
    // Sealed secret: "<iv>:<ciphertext>", both base64url. Never returned to a client.
  ...
  (table) => ({
    // One credential per provider per user - saving again updates in place.
    userProviderIdx: uniqueIndex("cloud_credentials_user_provider_idx").on(
      table.userId,
      table.provider,
    ),
  }),
