/**
 * Cookie consent: the record the banner writes, and the storage around it.
 *
 * Consent belongs with the Terms and the Privacy Policy rather than with any
 * one app's UI — it is the same promise, made in a smaller box, and it links
 * to the document this package renders. Kept framework-free so analytics
 * loaders, server code and tests can read the visitor's choice without
 * pulling React in; the banner itself is in `legal-terms/react`.
 */

/** The categories a visitor consents to, plus when they decided. */
export interface CookieConsentRecord {
  /** Usage measurement — page views, feature counts, error rates. */
  analytics: boolean;
 ...
 * browsers that throw on access when site data is blocked. Callers below read
 * that case as "no decision recorded", so the banner shows and nothing beyond
 * essential cookies is assumed.
 */
...
export function readCookieConsent(
  ...
  try {
    ...
    // the property access (Chrome with site data blocked) and the call itself
    // (Safari's private mode) — and both mean the same thing here.
    ...
    return {
      ...
      timestamp:
        typeof parsed.timestamp === 'string' ? parsed.timestamp : new Date(0).toISOString(),
    };
  } catch {
    return null;
  }
 ...
 * storage blocked the choice holds for this page view and the banner returns
 * on the next one.
 */
